...
export default function LocationInteractionScreen() {
  ...
  const handleOpenChest = () => {
    ...
    dispatch({ type: 'SET_LOOT', payload: loot });
    navigate('/loot');
  };
    ...
    <GameShell>
      ...
      <ScreenTransition>
        <div className="flex flex-col items-center gap-6 p-6 flex-1">
          {/* Type badge */}
          <div
            className={`px-3 py-1 rounded-md border-2 text-xs font-display font-bold uppercase tracking-wide ${
              marker.type === 'monster'
                ? 'bg-[hsl(var(--game-red)/0.15)] border-[hsl(var(--game-red)/0.3)] text-[hsl(var(--game-red))]'
                : marker.type === 'chest'
                ? 'bg-[hsl(var(--game-yellow)/0.15)] border-[hsl(var(--game-yellow)/0.3)] text-[hsl(var(--game-yellow))]'
                : 'bg-[hsl(var(--game-purple)/0.15)] border-[hsl(var(--game-purple)/0.3)] text-[hsl(var(--game-purple))]'
            }`}
          >
            {marker.type}
          </div>
          ...
          <div className="w-full text-center">
            ...
            {monster && (
              <div className="inline-flex items-center gap-1.5 bg-muted border-2 border-border rounded-md px-3 py-1 mb-4">
                ...
                <span className="text-xs text-[hsl(var(--game-red))] font-display font-bold">
                  ATK {monster.attack}
                </span>
                <span className="text-muted-foreground">·</span>
                <span className="text-xs text-[hsl(var(--game-green))] font-display font-bold">
                  DEF {monster.defense}
                </span>
              </div>
            )}
          ...
          {monster && (
            <div className="w-full bg-muted/50 border-2 border-border rounded-lg p-3 flex gap-4 justify-center">
              <div className="text-center">
                ...
                <p className="font-display font-bold text-sm text-[hsl(var(--game-yellow))]">
                  +{monster.goldReward} 🪙
                </p>
              </div>
